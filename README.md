...
Browse pages show 50 objects. Generated blocks show up to 100 rows with an explicit truncation notice. Refine the prompt to narrow larger result sets. Object search and writing-link search return at most 50 matches and explicitly ask you to narrow truncated results. Reference pickers are bounded to 200 live candidates per target type; view input pickers show up to 200 candidates. Existing selections remain visible. Calendar agendas group by the stored start date, not a month grid or recurrence engine.
