...
function pick(resource: Resource, workspace?: string): string {
  return workspace ? `<label class="pick"><input type="checkbox" name="selected" form="chat-send" value="${e(resource.href)}" aria-label="Select ${e(resource.facts.ID)}"><span class="sr-only">Select ${e(resource.facts.ID)}</span></label>` : '';
}
...
function detail(resource: Resource, workspace?: string): string {
  return `${pick(resource, workspace)}<p class="eyebrow">IN FOCUS / ${e(resource.facts.ID)}</p><h2>${e(resource.title)}</h2><p class="description">${e(resource.description)}</p><dl class="facts">${Object.entries(resource.facts).map(([key, value]) => `<div><dt>${e(key)}</dt><dd>${e(value)}</dd></div>`).join('')}</dl>`;
}
...
export function resourcePage(resource: Resource, visitor: Visitor, workspace?: Workspace, saved = false): string {
  ...
  return page(resource.title, workspace ? workbench(workspace, visitor, canvas, resource.href) : canvas, Boolean(workspace));
}
