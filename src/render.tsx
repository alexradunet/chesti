...
function ConversationPanel({ workspace, visitor, focus = '' }: { workspace: Workspace; visitor: Visitor; focus?: string }) {
  ...
  return <aside id="conversation" tabindex={-1} aria-label="Conversation with Pi" data-busy={String(Boolean(running))}><div class="conversation-heading"><div><span class="eyebrow">WORKSPACE ASSISTANT</span><h2>Conversation</h2></div><a class="conversation-jump" href="#canvas">Workspace ↑</a></div>
    <div id="transcript" tabindex={0} aria-label="Conversation history"><article class="message user"><span class="speaker">YOUR FOCUS</span><p>{workspace.task}</p></article><article class="message desk"><span class="speaker">GET STARTED</span><p>{workspace.kind === 'today' ? 'Plan your day or update approved records.' : 'Ask about issues, assign work, or change your view.'} Select items in the workspace to include them in your message.</p></article>
    ...
    <div class="conversation-input"><p id="chat-status" role="status">{running ? 'Working on your request…' : 'Ready when you are.'}</p><p id="selection-status" class="fine">No selection · using the current view.</p>
      <form id="chat-send" method="post" action={`/workspaces/${workspace.id}/messages`}><Hidden name="csrf" value={visitor.csrf} /><Hidden name="requestId" value={randomUUID()} /><Hidden name="revision" value={workspace.revision} /><Hidden name="focus" value={focus} /><label for="message">Your message</label><textarea id="message" name="message" rows={3} maxlength={2000} required aria-describedby="composer-hint" placeholder={workspace.kind === 'today' ? 'Create a task to finish the homepage tomorrow' : 'Assign both issues to me'} disabled={Boolean(running)}></textarea><p id="composer-hint" class="fine composer-hint">Demo supports sample requests. Choose Pi for open-ended help.</p><div class="chat-controls"><label class="sr-only" for="chat-engine">Conversation engine</label><select id="chat-engine" name="engine" disabled={Boolean(running)}><option value="pi" selected={conversation.engine === 'pi'}>Pi · AI assistant</option><option value="demo" selected={conversation.engine === 'demo'}>Demo · no AI</option></select><button class="primary" disabled={Boolean(running)}>Send message</button></div></form>
      <form id="chat-stop" method="post" action={`/workspaces/${workspace.id}/stop`} hidden={!running}><Hidden name="csrf" value={visitor.csrf} /><Hidden name="turnId" value={running?.id ?? ''} /><button>Stop response</button><span class="fine">Applied actions remain.</span></form><details class="privacy-note"><summary>Privacy &amp; permissions</summary><p class="fine">Actions run as Alex in this local sandbox. Pi sends conversation and inspected {workspace.kind === 'today' ? 'approved app' : 'issue'} data to your model provider. Sensitive actions require confirmation. Demo does not call an AI model.</p></details></div>
  </aside>;
}
...
function workbench(workspace: Workspace, visitor: Visitor, canvas: string, focus = ''): string {
  return (<div class="workbench" data-workspace={workspace.id}><section class="canvas-region" aria-label="Task workspace"><div id="pending-update" role="status" hidden>The workspace changed. Your unfinished form has been kept. <button type="button" id="apply-update">Discard form edits and refresh</button></div>{raw(canvas)}</section><ConversationPanel workspace={workspace} visitor={visitor} focus={focus} /></div>).toString();
}
